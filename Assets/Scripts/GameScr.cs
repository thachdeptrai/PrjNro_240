...
public class GameScr : mScreen, IChatable
{
    ...
    public override void update()
    {
        ...
        try
        {
            ...
            {
                long num = (long)dHP - (long)Char.myCharz().cHP >> 1;
                if (num < 1)
                {
                    num = 1;
                }
            ...
            {
                long num2 = (long)dMP - (long)Char.myCharz().cMP >> 1;
                if (num2 < 1)
                {
                    num2 = 1;
                }
    ...
    private void paintInfoBar(mGraphics g)
    {
        ...
        else
        {
            ...
            else if (Char.myCharz().mobFocus != null)
            {
                ...
                {
                    mFont.tahoma_7b_green2.drawString(g, NinjaUtil.FormatNumber(Char.myCharz().mobFocus.hp) + string.Empty, imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
                }
            }
            ...
            else if (Char.myCharz().charFocus != null)
            {
                ...
                mFont.tahoma_7b_green2.drawString(g, NinjaUtil.FormatNumber(Char.myCharz().charFocus.cHP) + string.Empty, imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
            }
            else
            {
                mFont.tahoma_7b_green2.drawString(g, Char.myCharz().cName, imgScrW / 2, 9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
                mFont.tahoma_7b_green2.drawString(g, NinjaUtil.FormatNumber(Char.myCharz().cPower) + string.Empty, imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
            }
        }
        g.translate(-g.getTranslateX(), -g.getTranslateY());
        if (isVS() && secondVS > 0)
