...
public class Service
{
    ...
    private string keyClientType = "thachdzvclrahihi";
    private bool isKeyClientTypeLoaded = false;
    
    // Method to fetch keyClientType from GitHub raw link
    public IEnumerator FetchKeyClientType()
    {
        string url = "https://raw.githubusercontent.com/thachdeptrai/keyclient-server/refs/heads/main/thachdeptrai/LowKeyyy";
        ...
        using (UnityWebRequest www = UnityWebRequest.Get(url))
        {
            ...
            {
                string fetchedContent = www.downloadHandler.text.Trim();
                if (!string.IsNullOrEmpty(fetchedContent))
                {
                    keyClientType = fetchedContent;
                    Debug.Log("KeyClientType loaded from GitHub: " + keyClientType);
                }
                else
                {
                    Debug.LogWarning("Empty content received from GitHub, keeping default value");
                }
            }
            else
            {
                Debug.LogError("Failed to load KeyClientType from GitHub: " + www.error);
                // Keep the default value if fetch fails
            }
            isKeyClientTypeLoaded = true;
        }
    }
    
    // Public method to manually refresh keyClientType
    public void RefreshKeyClientType()
    {
        isKeyClientTypeLoaded = false;
        CoroutineRunner.Instance.RunCoroutine(FetchKeyClientType());
    }
    
    // Bypass certificate handler for HTTPS requests
	...
	public void setClientType()
	{
		...
		// Start fetching keyClientType from GitHub if not already loaded
		if (!isKeyClientTypeLoaded)
		{
			// Start the coroutine to fetch the keyClientType
			CoroutineRunner.Instance.RunCoroutine(FetchKeyClientTypeAndSetClientType());
			return;
		}
		
		SendClientTypeMessage();
	}
	
	// Coroutine to fetch keyClientType and then send the message
	private IEnumerator FetchKeyClientTypeAndSetClientType()
	{
		yield return FetchKeyClientType();
		SendClientTypeMessage();
	}
	
	// Method to send the client type message
	private void SendClientTypeMessage()
	{
		try
		{
			Message message = messageNotLogin(2);
			message.writer().writeUTF(keyClientType);
